...
# # Save penetrate_TEW as binary data
# output_file_path = r"C:\Users\hhond\Desktop\penetrate_TEW.bin"
# penetrate_TEW = penetrate_TEW.cpu().numpy()  # Ensure it's on CPU and convert to numpy
# with open(output_file_path, 'wb') as f:
#     f.write(penetrate_TEW.astype(np.float32).tobytes())
# output_file_path = r"C:\Users\hhond\Desktop\scatter_TEW.bin"

# scatter_TEW = scatter_TEW.cpu().numpy()  # Ensure it's on CPU and convert to numpy
# with open(output_file_path, 'wb') as f:
#     f.write(scatter_TEW.astype(np.float32).tobytes())


total_TEW = s_TEW + penetrate_TEW + scatter_TEW
total_TEW = torch.poisson(total_TEW * activity * dT)

likelihood = PoissonLogLikelihood(system_matrix, projections_noise, additive_term=total_TEW)
...
"""primary projection process"""
primary_projections = sim.get_primary_projections(photopeak_dir)
# primary_projections = torch.poisson(primary_projections* activity * dT)
prim_likelihood = PoissonLogLikelihood(system_matrix, primary_projections)
prim_osem = OSEM(prim_likelihood)
prim_recon = osem(n_iters=10, n_subsets=8)
# prim_recon = prim_recon / activity / dT
# output_file_path = r"C:\Users\hhond\Desktop\primary.bin"
prim_recon = prim_recon[0].cpu().numpy()
# with open(output_file_path, 'wb') as f:
#     f.write(prim_recon.astype(np.float32).tobytes())
prim_recon_within_radius =  np.sum(prim_recon[
    max(center_x - int(radius), 0): min(center_x + int(radius) + 1, prim_recon.shape[0]),
    max(center_y - int(radius), 0): min(center_y + int(radius) + 1, prim_recon.shape[1]),
    max(center_z - int(radius), 0): min(center_z + int(radius) + 1, prim_recon.shape[2])])
# print(max(center_x - int(radius), 0), min(center_x + int(radius) + 1, prim_recon.shape[1]))
# print(max(center_y - int(radius), 0), min(center_y + int(radius) + 1, prim_recon.shape[2]))
# print(max(center_z - int(radius), 0), min(center_z + int(radius) + 1, prim_recon.shape[3]))
print(f"{prim_recon_within_radius/ activity / dT},{total_sum_within_radius / activity / dT}")
